...
class Summarizer:
    ...
    def _create_filter_chain(self, model_id: LanguageModelId):
        model_info = self.llm_factory.get_model_info(model_id)
        if not model_info:
            raise ValueError(f"Invalid model ID: '{model_id}'")
        filtering_llm = self.llm_factory.get_model(
            model_id=model_id, max_tokens=model_info.max_output_tokens, temperature=0.0
        )
        return (
    ...
    def _create_summarizer_chain(
        ...
        summarization_llm = self.llm_factory.get_model(
            model_id=model_id,
            max_tokens=summarization_info.max_output_tokens,
            temperature=0.0,
        )
        return (
    ...
    def _filter_posts(
        ...
        responses = self.batch_processor.execute_with_fallback(
            ...
            batch_func=self.filter_chain.batch,
            sequential_func=self.filter_chain.invoke,
            task_name="filtering",
        )
        filtered_posts = []
