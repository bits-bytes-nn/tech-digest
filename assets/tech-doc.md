...
- **AWS 아키텍처** (인프라 & 데이터 흐름):
  [`assets/diagrams/aws-architecture.png`](diagrams/aws-architecture.png)
  (소스: `aws-architecture.drawio`).
- **처리 파이프라인** (수집 → 전달, 손글씨 스타일):
  [`assets/diagrams/pipeline-flow.png`](diagrams/pipeline-flow.png)
  (소스: `pipeline-flow.excalidraw`).
...
- `min_content_length`(기본 600, ≥0) — **콘텐츠 충분성 임계값**(§11 참고). 마크업을
  제거한 가시 텍스트 길이가 이보다 짧으면 요약 전에 제외됩니다.
...
- `validate_tags`는 태그를 중복 제거·정렬·상한 적용.
...
### 날짜 헬퍼
- `parse_published_date` — ISO-8601 우선, 이후 알려진 포맷 목록 시도. 항상
  타임존 인식 UTC datetime 반환(실패 시 "현재"로 폴백).
...
모델의 요약 출력을 검증: 마크다운을 HTML로 변환(스타일용 클래스 주입 포함), 태그
정규화(중복 제거/상한/기본값), URL 정규화(마크다운 링크 → 앵커).
...
- **`process_posts`**가 공개 진입점이자 품질의 단일 초크 포인트: ① **콘텐츠
...
- **`_postprocess_summary`** — 알려진 수정(한글 콜론→마침표, 소스 URL 정규화)을
  작고 **선언적인** 치환 맵(`_SUMMARY_REPLACEMENTS`)으로 적용. 이전엔 인라인
  `.replace()` 체인이었으나 명시적이고 문서화된 맵으로 바꿨습니다.
...
- **`tests/`** — 빠르고 네트워크 없는 pytest 스위트. 날짜/소스/이미지 파싱,
  요청 계층(모킹 세션: 헤더 로테이션·일시 재시도·도메인별 캐시), 콘텐츠 게이트,
  크롤 헬스 분류, **프롬프트 캐싱 분할**(정적 프리픽스에 `{post}`가 없고, 백엔드별로
  올바른 캐시 마커만 들어가며, 양쪽 langchain_aws 변환 함수를 실제로 통과하는지),
  필터/요약 내부 로직(점수 경계·title 덮어쓰기·None 응답 정렬·잘못된 출력 처리·
  `max_posts` 요약-전 캡), thinking budget 클램프, Bedrock 모델 정보 레지스트리 및
  크로스 리전 ID 구성, 설정 검증, 요약 정규화/후처리, **AWS 헬퍼**(SES MIME 빌드·
  S3 키/프리픽스·404 분기·Batch 폴링 상태머신, 페이크 boto 클라이언트), **`run_batch`**
  의 `"null"` 센티넬 파라미터 처리·날짜 검증, **`main.handler`** 제어 흐름(무-게시물
  200 / 예외 500), 엔드투엔드 Jinja2 렌더링(점수 배지·다크모드·접근성·alt 텍스트),
  SNS 알림 배선을 커버. 전체 라인 커버리지 ~62%(핵심 경로 중심). `conftest.py`가
  `sys.path`를 고치고 공유 픽스처를 제공.
