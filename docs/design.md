...
│   └── check_docs_current.py    # 문서의 수치 주장을 실측과 대조 (변경 마지막 단계)
├── tests/                       # pytest 스위트
...
`_scrape_full_content`는 그 추가 페치를 수행하는데, **어느 컨테이너에 본문이 들어 있는지를
보고 고릅니다.** `CONTENT_SELECTORS`를 순서대로 훑어 처음 매칭된 것을 그대로 돌려주던
예전 방식은 매칭된 요소 안을 들여다보지 않았고, 그래서 어떤 셀렉터든 하나라도 걸리면
`<body>` 폴백은 **도달 불가**였습니다 — 빈 껍데기가 걸려도 마찬가지였습니다.

Hugging Face 블로그에서 실측했습니다. 그 페이지의 `<article>`은 헤딩만 담은 래퍼라
**가시 텍스트 82자**를 돌려줬고, 같은 페이지의 본문 **21,669자**는 버려졌습니다(다른
글에서는 93자 대 64,950자). 그렇게 얇아진 글은 §11의 게이트에 걸려 탈락하므로, 정상
동작하는 소스가 아무것도 기여하지 못했습니다. 2026-08-19 실행에서 수집한 26건 중
**21건이 이 경로로 사라졌습니다.**

지금은 모든 셀렉터의 모든 매칭 중 가시 텍스트가 가장 많은 것을 고르고, 그마저도 페이지
`<body>` 산문의 `CONTENT_CONTAINER_MIN_SHARE`(0.5)에 못 미치면 `<body>`를 씁니다. 좁은
컨테이너를 선호하는 이유는 내비게이션·푸터를 배제하기 때문인데, 그 값어치는 그 컨테이너가
기사를 실제로 담고 있을 때만 있습니다. HF 래퍼는 0.4%였고 `<article>`이 제대로 기사인
사이트는 보통 60\~90%라, 절반이라는 컷은 둘을 가르는 데 조정이 필요 없습니다.
...
**어느 필드에서 날짜를 읽는지도 게이트의 일부입니다.** `entry_date_text`가
`published`를 먼저 보고 없으면 `updated`로 떨어집니다. Atom에서 `<published>`는
**선택**이고 `<updated>`가 필수인데 feedparser는 둘을 서로 다른 속성으로 매핑하므로,
`<updated>`만 있는 정상적인 Atom 피드에는 `entry.published`가 **아예 없습니다**.
`published`만 읽던 예전 코드에서는 그런 피드의 모든 엔트리가 fail-closed 게이트에
걸려 영구히, 그리고 보이지 않게 버려졌습니다 — 후보는 정상으로 보고되니 §7의
판별기에도 "그냥 조용한 블로그"로 읽혔습니다.

지금 설정된 피드 19개를 실측하면 엔트리 2,815건 전부가 파싱 가능한 `published`를
갖고 있어 **이 폴백으로 되찾은 글은 없습니다.** 즉 이건 "NOTE: add new sources here"
경로에 놓인 함정을 없앤 것입니다. 둘 다 있으면 `published`가 이깁니다. `updated`는
수정 시각이므로 그쪽을 우선하면 이번 주에 손본 옛 글이 주간 윈도우에 다시 들어옵니다.
...
- `SourceStatus`. `OK`(가져와서 글까지 생성), `EMPTY`(가져왔지만 기간 안에
  글이 없음), `FAILED`(가져오기 자체가 실패).
- `SourceHealth`. 소스별 레코드: `url`, `fetcher`, `status`, `post_count`,
  `duplicates`, `candidates`, `error`.
- `CrawlReport`. `SourceHealth`들을 한데 모읍니다. `failed` / `empty` / `ok` /
  `likely_broken`으로 나눠 보여 주고, `total_posts`, 한 줄 요약 `summary_line()`,
  그리고 사람이 읽기 좋게 정리한 `format_alert()`를 제공합니다.

### `EMPTY`는 두 가지를 뭉개고 있었습니다

`EMPTY`("가져왔지만 기간 안에 글이 없음")에는 성질이 전혀 다른 두 상황이 섞여
있습니다.

1. **블로그가 이번 주에 글을 안 썼다.** 정상입니다. 할 일이 없습니다.
2. **피드가 이전했거나 스크레이퍼 셀렉터가 깨졌다.** 가져오기는 성공했으니 `FAILED`가
   아니고, 글은 0편이니 `EMPTY`입니다. 고쳐야 합니다.

둘이 리포트에서 **똑같은 한 줄**로 보였습니다. 그런데 보통 한 실행에서 19개 소스 중
12개가 `EMPTY`이므로, 깨진 크롤러 하나는 그 목록에 묻혀 몇 주를 버틸 수 있었습니다.
하필 마크업 변경 위험이 있는 인덱스 스크레이퍼 세 개의 실패 모드가 정확히 이것입니다.

구분하는 데 필요한 건 **날짜 필터 이전의 후보 개수**입니다. 조용한 블로그는 자기 과거
글을 그대로 내놓고(후보 14개) 날짜 조건이 그걸 걸러냅니다. 깨진 크롤러는 애초에 걸러낼
것을 하나도 못 내놓습니다(후보 0개). 그래서 `SourceHealth.candidates`를 기록하고,
`CrawlReport.likely_broken`이 `EMPTY` 중 후보가 0인 것만 골라냅니다.

- 이 숫자는 **원래 모든 fetcher가 계산해서 버리던 것**입니다. `RssFetcher`는
  `len(feed.entries)`, `GenericPageScraper`는 `len(soup.select(ITEM_SELECTOR))`,
  인덱스 스크레이퍼는 `LINK_PATTERN`에 매칭된 링크 수입니다. Meta 스크레이퍼는
  로그에 찍기까지 했습니다.
- `PostFetcher` 프로토콜에 선언하되, 수집기는 `source_url`과 마찬가지로 `getattr`로
  방어적으로 읽습니다. 보고하지 않는 fetcher는 `None`이 됩니다.
- **`None`은 0이 아닙니다.** "보고 안 함"을 "0개"로 취급하면 모르는 경우에 경보가
  울립니다. `likely_broken`은 `candidates == 0`만 셉니다.
- **네 번째 `SourceStatus`로 만들지 않았습니다.** "가져왔고 기간 내 글은 없다"는
  상태 서술로서 정확하고, 새로운 건 그 이유를 가르는 **증거**입니다. 그래서 추론은
  파생 속성에 두고, 모든 호출자가 처리해야 하는 상태를 늘리지 않았습니다.
- **실행 간 상태가 필요 없습니다.** 원래 §23에 적어 둔 계획은 "연속 몇 주 `EMPTY`인지
  세자"였고 그러려면 S3에 지난 실행 기록을 두어야 했습니다. 그렇게 만들어도 결론은
  "eugeneyan이 4주째 조용합니다" 같은 **조치 불가능한 알림**입니다. 실제로 조치가
  필요한 두 경우는 한 실행 안에서 판정됩니다.

리포트 본문이 이유를 함께 적습니다.

```
Empty sources (no posts in window):
  - https://eugeneyan.com/rss/ [RssFetcher]: offered 14 item(s), none in the date window
  - https://ai.meta.com/blog/ [MetaAIBlogScraper]: offered 0 items — LIKELY BROKEN (moved feed or stale selector)
```

이 리포트는 `PostCollector.collect_posts`가 실행하면서 채웁니다. AWS에서
실행하다 소스가 하나라도 `FAILED`가 되거나 `likely_broken`이 되면, `main.py`가 곧바로
전용 SNS 알림을 발행합니다(`_send_crawl_health_alert`). 덕분에 깨진 소스(예: 데이터센터
IP에서만 발동하는 안티봇 차단, §21)가 다이제스트에서 조용히 사라지는 대신 바로 손쓸 수
있는 이메일이 됩니다. **조용한 블로그는 절대 이 알림을 띄우지 않습니다** — 후보가
0이 아니기 때문입니다. 커버리지보다 이게 더 중요합니다. 매주 비어 있는 12개 소스에
울리는 알람은 아무도 읽지 않는 알람이고, 그건 `expected_flaky_urls`가 막으려는 실패와
같은 것입니다.
...
나오지만 **두 트리거 모두에서 제외**합니다(x.ai가 차단당한 결과가 `FAILED`로 나오든
후보 0개로 나오든 울리지 않아야 합니다).

**실측(2026-08-19, dev 엔드투엔드).** 봇 보호가 걸린 소스 셋이 AWS 송신 IP에서 전부
아무것도 내놓지 못하고, 실패 **모양이 서로 다릅니다.**

| 소스 | 비데이터센터 IP | AWS | 상태 |
| --- | --- | --- | --- |
| `x.ai/news` | (미측정) | 요청 자체 실패 | `FAILED` |
| `ai.meta.com/blog/` | 후보 21개 | 200 + 후보 0개 | `EMPTY`, likely_broken |
| `www.anthropic.com/engineering` | 후보 25개 | 200 + 후보 0개 | `EMPTY`, likely_broken |

셋 다 `expected_flaky_urls`에 있습니다. 헤더로 해결될 문제가 아닙니다 — 래더가 이미
현실적인 브라우저 지문 3종을 보내고, 막는 것은 ASN입니다. 억제해도 **기록은 남습니다**:
리포트 본문은 `offered 0 items — LIKELY BROKEN`을 계속 적고, `PostCollector`가 매 실행
그 URL을 WARNING으로 남깁니다. 조용해지는 것은 트리거뿐입니다.

인덱스 스크레이퍼 6개 중 **3개가 AWS에서 기여하지 못합니다**(위 셋). Google과 Qwen은
정상 동작하고 LinkedIn은 dev 목록에 없어 미검증입니다. 이 셋을 계속 둘지는 유지 비용
대비 기여도 판단이고 §23에 남겨 두었습니다. 실패가 전부 예상된 것이면 알림을 보내지 않고
...
> `is_korean`(한글 음절 비율)을 두고,
> `eval_summary_quality.py`가 한국어가 아닌 실행을 **채점하지 않고 거부**합니다.
> `inputs/`에 저장된 요약 62건을 실측하면 한글 비율이 0.41\~0.89(중앙값 0.75, p10
> 0.62)이고 영문 요약은 0.00입니다. 임계값 0.1은 관측 최솟값보다 한참 아래에
> 둡니다 — 판정해야 하는 건 "한국어인가"뿐이고, 식별자·모델명·단위가 유독 많은
> 한국어 요약까지 여유롭게 통과시켜야 하기 때문입니다.
...
> **게이트는 추출 결함을 가릴 수 있습니다.** 2026-08-19 엔드투엔드 실행에서 게이트가
> 수집 26건 중 **21건**을 버렸고 필터에는 5건만 도달했습니다. 원인은 글이 실제로 빈약해서가
> 아니라 §6의 컨테이너 선택이 잘못돼 HF 기사 본문을 82자로 읽고 있었기 때문입니다. 게이트는
> 제 일을 했지만, 그 로그("141 visible chars < 600 minimum")는 **글 탓으로 읽힙니다.**
> 게이트가 평소보다 많이 버리면 임계값을 내리기 전에 **추출부터 의심하세요** — 탈락한 글의
> URL을 열어 `_scrape_full_content`가 몇 자를 얻는지 재 보는 것이 순서입니다.
...
- `read_s3_json` / `write_s3_json`. 작은 JSON 객체를 S3에서 읽고 씁니다(전달 원장이
  씁니다). 읽기는 "객체 없음"과 "읽기 실패"를 **모두 `None`**으로 돌려줍니다. 지금까지
  모든 호출자가 둘을 같게 처리하기 때문이고, 어느 쪽이었는지는 로그가 말합니다. 구분이
  필요한 호출자는 이 함수를 쓰면 안 됩니다.
...
8. 이메일이 켜져 있으면 수신자를 해석하고(CLI 인자 또는 S3 파일), **이미 이 호를 받은
   수신자를 건너뛴 뒤**(아래 "전달 원장"), 수신자마다 약간의 간격을 두고 발송합니다. 전원
   발송에 성공하면 알림을 보내지 않고, 일부 수신자가 실패했을 때만 부분 실패 경보
   (`_maybe_send_partial_delivery_alert`)를 발행합니다(크롤 요약과 제외 글 포함).
   결과는 `DeliveryOutcome`(해석된 수, 건너뛴 수, 시도한 수, 성공 수, 실패 목록)으로
...
두 재시도 경로(Batch 잡 정의와 EventBridge 타깃의 `retry_attempts=2`)는 모두
`main.py`를 **처음부터** 다시 돌립니다. 그래서 발송 루프 뒤 어디서든 잡이 죽으면 같은
호가 두 번 발송됐습니다. 스팟을 우선 컴퓨트 환경으로 올린 뒤로는 스팟 회수가 그
경로의 가장 흔한 원인입니다(§18).

가드는 **수신자 단위**입니다. 피해가 측정되는 단위가 수신자이기 때문이고, 덤으로
루프 중간에 죽은 경우가 이전보다 *더 나아집니다*: 재시도가 아직 못 받은 독자에게만
보냅니다. 호 단위 마커라면 "이 호는 끝났다"고 주장해서 못 받은 독자를 영구히
빠뜨렸을 것이고, 마커가 없으면 전원에게 다시 보냅니다.

- **키**: `{s3_prefix}/deliveries/delivered-{stage}-{date}-{language}.json`.
  호를 식별하는 것이 정확히 이 세 조각이고, 렌더된 파일명도 같은 조각을 씁니다.
- **쓰기 시점**: 성공한 발송 **하나하나 뒤에**. 루프 끝에 한 번만 쓰면 정작 필요한
  상황(중간에 죽은 경우)에 원장이 없습니다.
- **실패한 발송은 기록하지 않습니다.** 기록하면 재시도가 못 받은 독자를 건너뜁니다.
- **비교는 소문자로.** 이메일 주소는 실질적으로 대소문자를 구분하지 않습니다.
- **fail open**: 원장이 없거나 읽을 수 없거나 형태가 깨졌으면 빈 집합으로 보고
  전원에게 보냅니다. 중복 한 통은 불편이고, 아무도 받지 못하는 호는 이 파이프라인이
  막으려는 실패입니다. 어느 쪽으로 틀릴지를 고른 것입니다.
- **`--recipients` 실행은 원장을 읽지도 쓰지도 않습니다.** 그건 운영자가 특정
  메일박스를 의도적으로 지정한 것(테스트 발송, 한 명 재발송)입니다. 원장이 관장하는
  것은 목록 기반의 정식 호이고, 재시도가 중복시킬 수 있는 것도 그것뿐입니다. 이
  구분이 없으면 자기 주소로 테스트 발송을 한 번 한 뒤 정작 정식 발송에서 자신이
  빠집니다.
- IAM은 그대로입니다. 원장은 이미 부여된 `s3:GetObject`/`s3:PutObject`만 씁니다.

남은 비용 하나: 발송 뒤에 죽은 잡의 재시도는 **필터링·요약을 다시 지불합니다**.
원장은 발송만 막고, 이 실행은 아무것도 보내지 않은 채 끝납니다. 이걸 없애려면
수신자 해석을 빌드 앞으로 당겨 조기 종료해야 하는데, 그건 별개의 변경입니다.
...
`tests/`는 638개 테스트에 커버리지 86.0%이고, 네트워크를 타지 않아 몇 초면 끝납니다.
...
### 문서 최신화 검증 (`scripts/check_docs_current.py`)

**변경의 마지막 단계입니다.** 문서의 수치 주장을 눈으로 읽지 않고 그 수치를 만드는
산출물과 대조합니다 — 테스트 수와 커버리지(스위트를 실제로 돌려서), 락의 고정 의존성
개수, `expected_flaky_urls`에 억제된 소스가 문서에 설명돼 있는지, 매칭되는 소스가 없는
플레이스홀더가 그렇게 표시돼 있는지. 어긋나면 non-zero로 끝나므로 커밋 게이트로 쓸 수
있습니다.

이 스크립트가 존재하는 이유는 같은 실수가 반복됐기 때문입니다. 락이 65개로 줄어든 뒤에도
README 두 개가 몇 주 동안 71개라고 적고 있었고, 알림 종류가 여섯인데 "네 종류"라고 적혀
있었고, 크롤 헬스 트리거가 둘이 된 뒤에도 설명은 하나만 다루고 있었습니다. 매번 코드가
맞고 산문이 조용히 틀렸습니다. **검증되지 않는 산문은 없는 산문보다 나쁩니다.** 신뢰받기
때문입니다.

테스트로 덮을 수 있는 것(마크다운 함정, 락 개수, README 구조 대칭, 템플릿 설정 키)은
`tests/test_docs.py`에 있습니다. 테스트 **수** 자체와 **커버리지**는 커버리지 실행 안에서
알 수 없어 서브프로세스가 필요하고, 그래서 스크립트로 분리했습니다.
...
- **일부러 같은 호를 다시 보내야 할 때:** 두 가지 길이 있습니다. 특정 주소로만 보내려면
  `--recipients you@example.com` — 지정 발송은 원장을 읽지도 쓰지도 않으므로 몇 번이든
  됩니다. **목록 전체에** 다시 보내려면 그 호의 원장을 지워야 합니다:
  `aws s3 rm s3://<bucket>/<prefix>/deliveries/delivered-<stage>-<date>-<lang>.json`.
  런타임 역할에는 `s3:DeleteObject`가 없으므로(앱은 객체를 지우지 않습니다) 운영자
  자격증명으로 지우세요. 지우지 않고 목록 발송을 다시 돌리면 전원이 건너뛰어지고
  "이미 다 받았다"는 로그만 남습니다.
...
- **로그의 `cache_write`는 항상 0입니다 — 캐시가 안 써졌다는 뜻이 아닙니다.**
  `ChatBedrockConverse`가 `usage_metadata`에 `cache_creation`을 보고하지 않아서
  `TokenUsageLogger`가 0을 찍습니다. 캐시가 도는지는 `cache_read`로 판단하세요.
  2026-08-19 실행에서 필터링 11건이 3청크로 돌았고 **첫 청크 5건은 read=0, 이후 6건은
  전부 read=1,571**이었습니다. 캐시 프리픽스가 1,571토큰으로 Bedrock 최소치(1024)를
  넘고, 정상 동작합니다. 5건짜리 실행에서는 1청크라 전부 냉 캐시이므로 이 숫자로는
  아무것도 판단할 수 없습니다.
...
- **커버리지.** 전체 86.0%입니다. `feed_parser`의 사이트별 스크레이퍼 내부 휴리스틱과
...
- **`min_score`가 앵커 경계에 얹혀 있습니다.** "OTHER ML CONTENT" 밴드는 0.80에서
  ...
  **단, 발행 편수가 적은 원인을 여기로 돌리던 예전 서술은 틀렸습니다.** 2026-08-19
  실측에서 병목은 필터가 아니라 **콘텐츠 게이트**였습니다: 26건 수집 → 게이트 통과 5건 →
  필터 통과 3건. 즉 `max_posts`도 `min_score`도 구속 조건이 아니었고, §6의 추출 결함이
  후보를 먼저 말려 버리고 있었습니다. 그 결함을 고친 뒤 편수를 다시 재야 이 항목을
  판단할 수 있습니다. 이는 결함이라기보다
  설정된 품질 기준이지만, 통과 편수를 늘리려면 임계값을 0.75로 낮추는 편이 앵커를 더
  쪼개는 것보다 낫습니다(앵커를 쪼개면 예전의 과적합 루빅스로 되돌아갑니다). 위의
  비재현성과 겹치면 경계 글의 편차가 더 잘 드러납니다.
- **재시도는 LLM 비용을 다시 지불합니다.** 중복 *발송*은 수신자 단위 전달 원장으로
  막았습니다(§16). 하지만 발송 뒤에 죽은 잡의 재시도는 크롤·필터링·요약을 처음부터 다시
  돌린 뒤 원장을 보고 "보낼 사람 없음"으로 끝납니다. 독자에게는 아무 영향이 없고 낭비되는
  건 한 번의 Bedrock 비용입니다. 없애려면 수신자 해석을 빌드 앞으로 당겨 조기 종료해야
  하는데, 지금 구조는 요약이 끝난 뒤에야 수신자를 해석하므로 순서를 바꾸는 별개의
  변경입니다.
- **평가셋 규모.** 현재 라벨링된 평가셋은 합성 글 6건으로, 회귀 게이트로는
  충분하지만 통계적으로는 작습니다. 실제 기사 발췌로 확장하면 밴드 경계 보정이
  더 정밀해집니다.
- **prod은 소스 2개(AWS 블로그 + amazon.science)뿐입니다.** `filtering_criteria: amazon`인
  단일 벤더 다이제스트라 인덱스 스크레이퍼가 하나도 걸려 있지 않습니다. 그래서 §7의
  후보 판별기·봇 차단·`expected_flaky_urls` 논의는 **전부 dev 얘기**입니다. prod의
  `expected_flaky_urls`에 남은 `x.ai/news`는 매칭되는 소스가 없는 플레이스홀더이고, 주석에
  그렇게 적어 두었습니다. 두 스테이지의 소스 구성이 이렇게 다르다는 점은 dev에서 측정한
  수치를 prod에 옮겨 읽지 않도록 기억해 둘 필요가 있습니다.
- **인덱스 스크레이퍼 3개가 AWS에서 기여하지 못합니다.** Anthropic·Meta·xAI가 봇 차단으로
  0건입니다(§7의 표). 유지하기로 한 판단은 그대로지만, 근거가 "기여가 적다"에서
  "프로덕션에서 0이다"로 강해졌습니다. 되돌아볼 때 쓸 데이터입니다.
- **크롤러가 엉뚱한 것을 매칭하는 경우는 여전히 못 잡습니다.** 후보 0개는 이제
  경보가 됩니다(§7). 하지만 사이트가 마크업을 바꿨는데 `LINK_PATTERN`이 여전히 무언가에
  걸리면 후보 > 0이라 통과합니다. 후보는 있는데 제목·날짜 추출이 전부 실패하는 형태입니다.
  잡으려면 후보 대비 파싱 성공률을 지표로 하나 더 들여야 하는데, 지금은 값에 비해
  복잡도가 큽니다.
- **영구히 죽었지만 유효한 피드를 계속 내놓는 소스**는 후보가 0이 아니므로 구분되지
  않습니다. 저자가 블로깅을 그만둔 경우입니다. 다만 이건 사건이 아니라 **소스 목록
  관리** 문제이고, 답은 알람이 아니라 리포트를 가끔 들여다보는 것입니다. 그 잔여 케이스
  하나를 위해 실행 간 상태를 들이는 건 값이 맞지 않습니다.
