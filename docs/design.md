...
- **`LanguageModelId`** — Bedrock Claude 모델 ID 카탈로그입니다. 최신 모델(Sonnet
  5, Sonnet 4.6, Opus 4.6/4.7/4.8, Haiku 4.5)과 레거시 모델이 함께 들어 있으며,
  현재 활성 기본값은 Sonnet 5(필터·요약)와 Haiku 4.5(인사말)입니다. 새 모델을 추가할
...
있는 이메일이 됩니다. 모든 SNS 알림(크롤 헬스, 부분 실패, 완전 실패)은 `utils.py`의
공통 `format_alarm()` 헬퍼로 통일된 형식(제목 `[<project>] <event> — <STATUS>`,
정렬된 필드, 말미 UTC 타임스탬프)을 갖습니다. 노이즈를 줄이려고 **정상 발송 시에는
알림을 보내지 않고**, 수신자 일부라도 실패했을 때만 부분 실패 경보를 띄웁니다
(이때 크롤 요약 `summary_line()`을 함께 실어 보냅니다).
...
`filtering_enable_thinking`를 **False**로 둬 점수의 재현성을 높입니다. (구세대
모델에서는 확장 사고가 temperature=1.0을 강제해 결정성을 깨뜨렸고, 신세대 Sonnet 5는
샘플링 파라미터가 아예 없는 대신 적응형 사고가 실행마다 달라지므로 — 어느 세대든
필터링에서 사고를 끄는 편이 점수가 안정적입니다.) 요약은 사고를 그대로
...
그 주 글들이 실제로 다루는 주제와 **직접 연결**되도록(무관한 일반론·향수 템플릿
금지) 지시하며, 인사말 컨텍스트로 이번 주 글 제목들을 함께 넘깁니다. 길이는
EN은 50~70 단어, KO는 **한글 100~140자**로 따로 잡습니다(영어 단어 기준을 그대로
쓰면 한국어 출력이 목표치에 못 미치던 문제를 보정).
...
7. 이메일이 켜져 있으면 수신자를 해석하고(CLI 인자 또는 S3 파일), 수신자마다 약간의
   ...
   (크롤 요약과 제외 글 포함).
8. 처리되지 않은 예외가 나면 실패 알림을 발행하고 500을 반환합니다. 세 종류의 알림은
   모두 `format_alarm()`으로 형식을 통일합니다(§7).
...
- **`pyproject.toml`** 이 도구 설정을 한곳에 모읍니다: `pytest`(`unit`/
  ...
  `mypy`.
- **`tests/`** — 빠르고 네트워크를 타지 않는 pytest 스위트입니다(276개 테스트).
  커버하는 범위는 다음과 같습니다.
...
- **필터링 점수 평가 하버스** — `scripts/eval_filtering.py`가 라벨링된 평가셋
  ...
  "재현 가능한 점수"라는 목표와 앵커 점수제가 실측으로 확인됐습니다. (기본 모델이
  Sonnet 5로 올라갔으므로, 앵커 점수제 준수는 `--live`로 재측정해 확인하는 것이
  좋습니다.)
