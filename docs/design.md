...
> **주간 AI 기술 블로그 다이제스트** 뉴스레터 서비스의 설계 레퍼런스입니다.
...
3. **필터(Filter)** — LLM(Amazon Bedrock의 Claude)이 각 글을 읽고 ML 연구
   관련성과 품질을 점수로 매깁니다.
4. **랭킹(Rank)** — 점수 순으로 정렬해 상위 *N*개만 남깁니다(선택적으로 소스별
   다양성 캡 적용). 이 순서가 그대로 뉴스레터의 카드 순서가 됩니다.
5. **요약(Summarize)** — 살아남은 글마다 Claude를 한 번 더 호출해, 한 문장 리드와
   태그, 참고 링크까지 갖춘 구조화된 HTML 설명을 만듭니다. 분량은 메일 클립 한계를
   넘지 않도록 예산이 정해져 있습니다.
6. **인사말(Greet)** — 짧고 친근한 "Peccy" 인사말을 생성합니다.
7. **렌더(Render)** — Jinja2로 다크모드, 점수 배지, 읽는 시간, 현지화된 UI 문구를
   갖춘 반응형 HTML 이메일을 조립하고 크기가 클립 예산 안인지 확인합니다.
...
- `filtering_model_id` / `summarization_model_id` / `greeting_model_id` —
  단계별 Bedrock 모델 ID(Claude). 기본값은 차례로 Sonnet 5 / Sonnet 5 /
  Haiku 4.5입니다.
...
- **`SOURCE_MAPPING`** — 도메인이나 핸들을 표준 소스명으로 매핑합니다. 일반
  ...
  `source`가 **`max_per_source` 다양성 캡의 집계 키**라서 서로 다른 저자 셋을 한
...
찾습니다. 까다로운 지점은 **여러 게시물을 담은 조상**에 도달했을 때입니다. 그 조상 전체에
...
집계해 함께 실어, 대량 모델 실패(예: 모든 호출이 거부된 Sonnet 5 리그레션 — §8)와
...
구성합니다 — 온도(구세대에서 사고를 켜면 1.0으로 강제; 신세대는 온도 자체를 생략),
...
`filtering_enable_thinking`를 **False**로 둬 점수의 재현성을 높입니다. 요약은 사고를
그대로 유지합니다(점수와 무관하므로). `min_score`가 갖는 임계값의 의미는 변하지
않았습니다.

> **재현성은 현재 달성되지 않았습니다(측정됨, 2026-08-19).** 루빅스는 "같은 콘텐츠는
> 매번 같은 점수를 받아야 한다"고 명시하지만, 기본 필터링 모델인 Sonnet 5로는 그
> 계약이 지켜지지 않습니다. `LanguageModelInfo.supports_sampling_params=False`이므로
> 팩토리가 `temperature`를 **의도적으로 보내지 않고**(신세대 API가 거부합니다),
> 그러면 그리디 디코딩을 요청할 방법이 없어 동일 입력의 반복 채점이 실제로 달라집니다.
> 같은 하버스를 연속 3회 라이브 실행한 결과:
>
...
> | 3 | 83% | 100% | 100% | 0.25 |
>
> 원인이 루빅스가 아니라 모델의 샘플링임은 대조 실험으로 분리했습니다 — **동일한
> 평가셋·루빅스·반복 횟수**로 temperature를 받는 모델을 쓰면 σ=0.000, spread 0.00:
>
...
> 유지**하기로 결정했습니다 — 필터링 판단력을 재현성보다 우선한 것이며, 재현 가능한
...
함께 한 문장 리드(`one_liner`), `tags`, 참고 `urls`를 요청합니다. **섹션 생략 규칙**을 둬서, 원문이 뒷받침하지
않는 섹션은 아예 빼게 했습니다 — "정보 없음" 같은 빈 섹션이 생기지 않도록.

콘텐츠 품질을 위해 몇 가지 원칙을 프롬프트에 못 박았습니다(발송본 감사에서 반복
관찰된 이슈를 원천에서 줄이려는 것입니다).
- **일관된 문체** — 한 뉴스레터는 여러 글을 묶어 보내므로, KO 요약은 모든 문장을
  **합니다체**로 통일하도록 지시합니다(원문 블로그 어조를 따라 한다체로 새지 않게).
  글마다 문체가 튀면 독자가 이질감을 느낍니다.
- **섹션 간 정보 차별화** — 📊 성과는 📌·🛠️에서 이미 제시한 수치를 재나열하는 곳이
  아니라 그 함의를 다루고, 🔮 향후는 🛠️의 한계를 뒤집어 재서술하지 말고 로드맵·기회를
  다루며 새로 덧붙일 게 없으면 생략하도록 했습니다.
- **📌 상투 문구 억제** — "~에게 주목할 만한 참고 자료입니다" 식 정형화된 마무리를
  금지해, 매주 같은 문구로 끝나는 보일러플레이트를 줄입니다.
- **섹션 헤딩** — 🔄 섹션 제목은 "핵심 아키텍처와 동작 방식"으로 확정했습니다(예전의
  "아이디어, 아키텍처, 또는 워크플로우 개요"는 템플릿 선택지가 출력에 노출된 흔적).
- **분량 예산(신규)** — 예전 지시는 "원문 길이의 최대 20%"였습니다. 이 상대 기준은
  양쪽으로 다 틀렸습니다. 긴 논문에서는 과도한 분량을 정당화하고, 짧은 글에서는 도달
  불가능한 목표가 되어 그냥 무시됐습니다(실측: 6.7KB 원문에 3.4KB 요약 = 51%). 이제
  **절대 예산**(KO 4,000~6,000자 / EN 1,300~2,000단어)에 섹션별 대략의 배분을 더해,
  원문 길이와 무관하게 카드 분량이 일정하게 나옵니다.

  > **한 번 과교정했다가 되돌린 이력.** 처음에는 이 예산을 KO 1,400~2,300자로 잡았는데,
  > 발행본이 **너무 짧고 기술 상세를 잃었다**는 피드백을 받았습니다. 원인 분석이 틀렸던
  > 게 아니라 처방이 과했습니다 — 클립을 유발한 건 대부분 **마크업**이었고(§14에서 49%
  > 감량) 요약 분량은 주범이 아니었습니다. 실측으로 재계산하니 `max_posts: 3`에서는
  > 글당 **8,000자까지도 89.7KB**로 한계 안이었습니다. 그래서 만족했던 이전 깊이(가시
  > 문자 평균 3,873자)를 웃도는 4,000~6,000자로 올리고, **🛠️ 기술 심층 분석을 가장 긴
  > 섹션(4~6단락)으로 명시**했습니다. "분량을 맞추려고 메커니즘·파라미터·측정값·설계
  > 트레이드오프를 희생하지 말 것"을 프롬프트에 못 박았습니다.
  >
  > 예산과 `max_posts`는 **한 쌍**입니다: 3편이면 6,000자가 72KB로 안전하지만 5편이면
  > 112KB로 잘립니다(5편은 약 4,000자). 테스트가 두 조합을 모두 고정하고, 예산을 다시
  > 과하게 낮추면 실패하는 반대 방향 테스트도 함께 둡니다.
- **한 문장 리드(`one_liner`, 신규)** — 카드마다 맨 위에 놓이는 한 문장입니다. 예전에는
  모든 카드가 📌 섹션의 긴 단락으로 시작해서, 여러 편이 실린 호를 훑는 독자가 무엇을
  읽을지 **골라낼 수가 없었습니다**. 프롬프트는 칭찬 형용사·"이 글은 ~를 다룹니다"류
  메타 서술·제목 반복을 금지하고, 메커니즘과 측정된 효과를 담으라고 요구합니다.

### `GreetingPrompt` / `GreetingRevisionPrompt`
"Peccy" 페르소나를 씁니다. `for_language`로 EN/KO를 고릅니다. "이번 주 tech fact"는
그 주 글들이 실제로 다루는 주제와 **직접 연결**되도록 지시하며, 인사말 컨텍스트로
이번 주 글 제목들을 함께 넘깁니다. 길이는 KO **100~150자**, EN **50~75단어**로 따로
잡습니다(영어 단어 기준을 그대로 쓰면 한국어 출력이 목표치에 못 미치던 문제를 보정).

**실측된 문제.** 2026-07-11 실행의 실제 인사말은 약 230자였고(목표 상한의 1.5배),
프롬프트가 명시적으로 **금지한** "과거엔 X … 이제 Y" 대조 템플릿을 그대로 썼습니다.
즉 이 프롬프트에서 가장 어려운 두 제약이 둘 다 지켜지지 않고 있었습니다. 대응은
두 갈래입니다.
1. 프롬프트에서 분량을 맨 앞 별도 블록으로 올리고("답하기 전에 직접 세어 보세요"),
   금지 패턴을 "모델이 기본값으로 집어드는 패턴이라서" 금지한다는 이유와 함께
   어떤 표현으로든 금지한다고 못 박았습니다.
2. 분량은 **코드가 정확히 셀 수 있는** 제약이므로, 프롬프트를 더 크게 외치는 대신
   생성 후 측정해 한 번 되먹입니다(`GreetingRevisionPrompt`, §12).
`KO_LENGTH_RANGE` / `EN_WORD_RANGE`는 `GreetingPrompt`의 ClassVar로 두어, 프롬프트가
말하는 숫자와 코드가 검사하는 숫자가 갈라질 수 없게 했습니다(테스트로도 고정).
...
- **`_sanitize_html`** — 요약은 Jinja `|safe`로 렌더되는 **신뢰 경계**입니다.
  ...
  `javascript:`/`data:` 스킴의 href·src를 걷어냅니다. `<code>`/`<pre>` 클래스는
  마크다운→HTML 변환 뒤 문자열 치환으로 주입하며, 그 *뒤*에 새니타이즈를 돌려 허용된
  클래스가 살아남게 합니다. **`<table>`은 문자열 치환 대신 여기서
  BeautifulSoup로 클래스(`table table-bordered`)를 강제 주입합니다** — 예전의 리터럴
  `"<table>"` 치환은 모델이 속성 붙은 `<table ...>`을 내보내면 놓쳐, 지표 표가
  이메일에서 테두리 없이 뭉개졌습니다. 실제 테두리·패딩 스타일은 템플릿의
  `.table-bordered` 규칙이 줍니다(§14).
...
### `SummarizerSettings`
`Summarizer`가 필요한 설정을 하나의 검증된 객체로 묶습니다. 예전에는 생성자가
**14개 파라미터**를 받고 `process_posts`가 그 위에 4개를 더 받았습니다. 필드 이름을
`summarization` 설정 섹션과 일치시켜 두었기 때문에, 컴포지션 루트(`main.py`)가
`SummarizerSettings.model_validate(cfg.summarization.model_dump() | {...})` 한 줄로
만들 수 있습니다 — 키를 다시 나열하지 않아도 되고, `src`가 `configs`를 임포트해
의존 방향이 뒤집히는 일도 없습니다(pydantic 기본 `extra="ignore"`).
...
- `__init__`에서 LangChain 체인 두 개를 만듭니다. **필터**
  ...
  **프롬프트 캐싱을 켭니다**(§9).
- **`process_posts(posts)`** — 공개 진입점이자 품질을 결정하는 단일 초크 포인트입니다.
  ① **콘텐츠 게이트**(§11) → ② 선택적 관련성 필터링 → ③ **라인업 선정**
  (`_select_for_digest`) → ④ 요약 순서로 처리합니다. 캡을 **요약 직전**에 적용하는
  것이 핵심입니다 — 요약은 LLM 단계 중 가장 비싸므로, 어차피 버려질 글을 요약하느라
  Bedrock 비용을 낭비하지 않으려는 것입니다.
- **`_select_for_digest`** — 랭킹과 라인업 결정을 한곳에 모았습니다. 정렬 키는
  `(score desc, published_date desc)`이며, 거친 앵커 점수 탓에 동점이 잦으므로
  최신순으로 결정적으로 가릅니다(같은 입력 → 항상 같은 호).
  `max_per_source`가 설정되면 1차 패스에서 캡을 적용하고, 캡 때문에 남은 자리는
  **랭크 순으로 백필**합니다. 발행 편수를 줄이는 캡은 실질적 손실(글이 줄어듦)을
  치르고 표면적 이득(다양성)을 얻는 교환이므로, 캡은 *어떤* 글이 자리를 채우는지만
  바꾸고 *몇 편*인지는 절대 바꾸지 않습니다. 미설정이 기본값인 이유는 단일 벤더
  다이제스트(`filtering_criteria: amazon`)에서는 소스 집중이 결함이 아니라 의도이기
  때문입니다.
- **`_gate_by_content_length`** — 빈약한 글을 제외하되, 사유와 함께
  `filtered_out_posts`에 기록해 실행 알림에 드러나게 합니다.
- **필터 입력은 마크업이 아니라 본문 텍스트**(`filter_on_visible_text`, 기본 True) —
  필터는 주제 적합성과 품질 등급을 매기고 그 정보는 **산문**에 있는데, 예전에는 원시
  HTML을 그대로 보냈습니다. 실측(2026-08-19): 필터 입력 토큰의 **약 87%가 마크업**
  이었습니다(한 페이지는 HTML 77,417자 안에 본문이 5,669자). 그리고 필터링은 이
  파이프라인 Bedrock 비용의 **74%**를 차지합니다 — 즉 청구서의 절반 이상이 내비게이션
  바와 인라인 스타일과 스크립트였습니다. 본문 텍스트만 보내도록 바꿔 **실행당 필터
  입력 387,961 → 51,156 토큰(-87%), 실행당 총비용 $1.245 → $0.648(-48%)** 이 됐습니다.
  요약기는 코드 블록·표·이미지 URL이 필요하므로 **여전히 전체 HTML**을 받습니다.
  (주의: 같은 주를 두 번 돌렸을 때 라인업이 한 편 달라졌는데, 그 글은 *설정이 동일한*
  두 실행에서도 0.70↔0.80으로 흔들렸던 항목입니다 — §9의 Sonnet 5 비재현성 탓이라
  이 변경 효과로 분리할 수 없습니다. 나머지 점수는 동일했습니다.)
- **`_filter_posts`** — 필터 체인을 배치로 실행하고, 각 글의 `score`/`reason`을
  ...
  (조용한 손실 방지). 본문이 빈 글(=`min_content_length: 0`일 때만 도달 가능)도
  이제 사유와 함께 기록해, 어느 목록에도 없이 사라지는 경로를 없앴습니다.
- **`_summarize_posts`** — 요약 체인을 배치로 실행하고, 각 글에
  `summary`/`one_liner`/`tags`/`urls`를 채운 뒤 **성공한 글의 리스트를 반환합니다**.
  예전에는 호출자가 "`post.summary`가 비어 있지 않은지"로 성공을 *추론*했는데,
  그 추론은 필드에 이미 텍스트가 있으면(RSS 티저 — §6) 무조건 참이 되어 실패한 글이
  성공으로 보였습니다. 성공 집합을 명시적으로 돌려주니 추론이 필요 없습니다.
- **이미지 출처 검증(`_strip_unsourced_images`)** — 모델은 원문에 있는 이미지만
  넣으라고 지시받지만, LLM은 그럴듯한 CDN 경로를 만들어 낼 수 있고 그러면 발송된
  메일에 **깨진 이미지**(혹은 덩그러니 alt 문자열)가 남습니다. 그래서 요약 안의
  `<img src>`는 **완전 일치 증거**로만 통과시킵니다: 우리 파서가 원문에서 뽑은
  `Post.images`(이미 절대 URL)에 있거나, 모델에게 준 원문 HTML에 문자열 그대로
  등장해야 합니다. URL 패턴을 추측하지 않으므로 오탐이 없고, lazy-loading처럼
  파서가 놓친 이미지도 두 번째 조건으로 살아남습니다. 참고 링크(`urls`)는 조금
  다르게 다룹니다 — 살짝 다듬어졌지만 실재하는 링크는 독자에게 여전히 유용하므로
  **제거하지 않고 경고만** 남깁니다(프롬프트 드리프트가 로그에 드러나게).
...
### 출력 품질 루빅스 (`app/src/quality_metrics.py`, `scripts/eval_summary_quality.py`)
`eval_filtering`은 **"올바른 글을 골랐는가"** 를 재고, 이쪽은 **"글이 잘 써졌는가"** 를
잽니다 — 독자가 실제로 받는 부분이고, 따라서 프롬프트 튜닝이 여기에 끌려가야 합니다.

모든 검사는 **결정적**이며 프롬프트의 특정 지시문과 1:1로 대응합니다. 그래서 낮은 점수는
"취향 문제"가 아니라 **어느 문장을 고쳐야 하는지**를 가리킵니다. LLM 심판은 "좋은가?"는
평가할 수 있지만 *프롬프트의 어느 문장을 바꿔야 하는지*는 알려주지 못하고 실행마다
흔들립니다.
...
**측정을 먼저 고쳐야 했던 일.** 첫 버전은 단위 없는 숫자까지 측정값으로 셌습니다. 그래서
"Gemini 3.6", "GPT-5.5" 같은 **버전 문자열**이 specificity를 부풀리고, 동시에 "📊가 앞
섹션의 수치를 반복한다"는 **유령 결함**을 만들어 냈습니다. 그 상태로 프롬프트를 고쳤다면
존재하지 않는 문제를 쫓았을 것입니다. 지금은 단위·%·비교 표현을 동반한 숫자만 측정값으로
인정하며, 이 함정은 테스트로 고정했습니다.

**튜닝 1회차 실측(2026-08-19).** 약한 차원 3개를 짚어 해당 지시문만 고치고 재생성해
비교했습니다.
...
바뀐 지시문은 세 개뿐입니다: ① 측정값은 단위와 함께 빠짐없이 옮기고 숫자를 형용사로
바꾸지 말 것, 원문에 코드가 있으면 최소 하나 싣고 수치 3개 이상 비교는 표로 할 것,
② 금지 문구를 열거해 "정보 없음" 섹션을 지우게 할 것, ③ lede 상한을 카드 레이아웃에서
유도한 값(두 줄 = 100자)으로 명시하고 세어 보게 할 것. 결과적으로 코드 블록 1→5개,
표 1→5개로 늘고 lede는 전부 밴드 안에 들어오면서 측정값을 담게 됐습니다.

주의: 두 실행의 선정 글 목록이 일부 다릅니다(§9의 Sonnet 5 비재현성). 통제된 A/B가 아니라
집계 방향과 메커니즘으로 읽어야 합니다.
...
- 수정본도 창을 벗어나면 **창에 더 가까운 쪽**을 고릅니다 — 수정이 인사말을 초안보다
  나쁘게 만들 수 없습니다.
...
언제나 발동했고 **영어 호도 `lang="ko"`로 선언**됐습니다(스크린 리더와 클라이언트
번역이 이 속성을 봅니다). `model_dump(mode="json")`으로 덤프해야 enum이 값으로
직렬화된다는 점도 함께 고정했습니다 — 아니면 속성에 `Language.KO`가 그대로 박힙니다.
...
잘라내고 "전체 메시지 보기" 링크를 남깁니다 — 즉 **뒤쪽 카드와 푸터가 독자에게
아예 보이지 않습니다**. 실제 발행본 18호를 측정해 보니 **8호가 이 한계를 넘겼고**,
...
쓰는 키와 **동일**합니다 — 선정과 제시가 같은 기준을 씁니다.
...
(157줄 / 334줄)와 각자의 카드 매크로**를 들고 있었습니다. 서로 다른 폰트
(Pretendard/Inter 대 Noto Serif KR/Lora), 다른 색, 다른 타입 스케일이었고 —
...
- **UI 문구 현지화** — `published_on` / `view_more` / `resources` / `reading_time` /
  ...
  템플릿에 영어가 하드코딩돼 있어서, **한국어 본문 주위를 "Published on" / "View
  More" / "Additional resources for reference"가 감싸고** 있었습니다. 라벨은
...
- **의존성 락(`app/requirements.lock`).** 베이스 이미지는 digest로 고정해 놓고 정작
  ...
  **CI도 이 락을 constraints로 씁니다**(`pip install -r requirements.txt -c
  ...
  설치하므로, **테스트가 운영에 도달하지 않는 버전을 검증**하게 됩니다 — 실측으로 핵심
  ...
  고정됐는지를 검사합니다 — 어느 하나가 어긋나면 조용히 지나가기 때문입니다.
...
- **`tests/`** — 빠르고 네트워크를 타지 않는 pytest 스위트입니다(324개 테스트).
  ...
  - 요약 정규화/후처리(태그 순서 보존, 종결 콜론 교정, **표 클래스 강제 주입**, 그리고
    **HTML 새니타이즈**·**URL 스킴 화이트리스트**의 XSS 가드).
...
- **필터링 점수 평가 하버스** — `scripts/eval_filtering.py`가 라벨링된 평가셋
  ...
  못 미치면 비정상 종료합니다. **라이브 측정 결과(Sonnet 5, 사고 OFF, 2026-08-19,
  3회 연속):** 0.05 그리드 준수 100%, 밴드 일치 100%, 그러나 결정성은
  100%/67%/83%로 흔들리고 개별 글의 spread가 최대 0.25까지 벌어졌습니다(§9의 대조
  실험과 원인 분석). 그래서 **게이트는 그리드·밴드 두 항목만** 판정하고, 결정성·밴드
  안정성·spread는 경고로 **보고**합니다. 기본 설정이 통과할 수 없는 항목을 게이트에
  넣으면 아무도 돌리지 않는 검사가 되기 때문입니다.
  밴드 라벨링 버그도 함께 잡았습니다: 밴드 컷오프가 루빅스 앵커와 독립적으로
  정의돼 있어 0.60을 "strong"으로 분류했는데, 루빅스는 0.60을 "Good", 0.70을
  "Strong"이라 부릅니다. 이제 `ANCHOR_BANDS`가 단일 출처이고 컷오프는 거기서
  파생되며, 테스트가 모든 앵커를 자기 라벨의 밴드에 고정합니다. 평가셋의
  `moderate-educational` 라벨도 `weak` → `moderate`로 교정했습니다(본문이 스스로
  "교육적으로 유용하나 신규성은 제한적"이라 서술하는데, 이는 0.35 앵커가 아니라
  0.50/0.60 앵커의 문구입니다).
- **`.github/workflows/ci.yml`** — push와 PR마다 실행합니다: 의존성 설치,
  ...
  포함), 커버리지 포함 `pytest`(**`--cov-fail-under=78` 래칫** — 목표가 아니라 퇴행
  방지선이며, 커버리지가 오르면 바닥도 함께 올립니다), 평가 하버스 **드라이런**.
  별도 잡이 **`cdk synth`**
  (차단형)를 **Lambda 경로(`config-ci.yaml`)와 Batch 경로(`config-ci-batch.yaml`)
  양쪽**으로 돌려 인프라가 컴파일되는지 검증하며(Batch가 운영 컴퓨트 토폴로지),
  `CDK_SYNTH_DUMMY_AZS=1`로 AWS 자격증명 없이도 VPC가 synth되게 하고
  `CDK_SYNTH_ONLY=1`로 시크릿 쓰기 경로를 건너뜁니다(§18).
...
- **커버리지.** 전체 81%입니다. `feed_parser`의 사이트별 스크레이퍼 내부 휴리스틱과
...
- **필터링 점수가 재현되지 않습니다(실측).** 루빅스는 "같은 콘텐츠는 매번 같은 점수를
  ...
  temperature를 받는 모델로 내리면 100%가 복원됩니다 — 상세 표는 §9.
