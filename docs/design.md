...
같은 결과가 나옵니다. `post_count`는 소스가 내놓은 수를 그대로 보고하므로, 중복 제거가
정상 소스를 `EMPTY`처럼 보이게 만들지도 않습니다.
...
정리하면 **번역투는 실측으로 개선되었고, 전체 점수는 유지되었으며, 상투구는 열린 문제로
남았습니다.** 다음에 손볼 곳은 `~라는 점입니다`인데, 명시적 금지가 통하지 않는 유일한
