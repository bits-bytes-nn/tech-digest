...
- **분량 예산(신규)** — 예전 지시는 "원문 길이의 최대 20%"였습니다. 이 상대 기준은
  양쪽으로 다 틀렸습니다. 긴 논문에서는 7,000자짜리 요약을 정당화해 메일이
  클립되고(§13), 짧은 글에서는 도달 불가능한 목표가 되어 그냥 무시됐습니다(실측:
  6.7KB 원문에 3.4KB 요약 = 51%). 이제 **절대 예산**(KO 1,400~2,300자 / EN
  900~1,600단어)에 **섹션별 배분**(📌 3~4문장 / 🔄 1~2단락 / 🛠️ 2~3단락 / 📊 1단락 /
  🔮 2~3문장)을 더해, 원문 길이와 무관하게 카드 분량이 일정하게 나옵니다. "원문이
  길다는 건 더 많이 쓰라는 뜻이 아니라 더 골라내라는 뜻"이고, 줄일 때는 기술적
  사실보다 중복 서술을 먼저 버리라고 명시했습니다.
